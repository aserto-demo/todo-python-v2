...
def requires_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        iss_url = issuer_url(oidc_issuer)
        ...
        if "azp" in claims and claims["azp"] != oidc_client_id:
            raise AuthError(f"'azp' claim '{claims['azp']}' does not match client ID", 401)

        if not isinstance(claims["sub"], str):
            raise AuthError(f"'sub' claim '{claims['sub']}'is not a valid identity", 401)
...
def find_signing_key(discovery_url: str, key_id: str) -> Key:
        ...
        for _ in range(2):
            ...
            if not keys:
                raise AuthError("Keyset missing required field 'keys': {keys}", 401)
...
def get_json(url: str) -> dict:  # type: ignore
